...
export default function ResetPasswordPage() {
  ...
  const [ready, setReady] = useState(false);          // جاهزية الجلسة بعد التقاط التوكن
  ...
  const tokens = useMemo(() => {
    ...
    // في كثير من الحالات Supabase يرسلها في الـ hash:
  ...
  useEffect(() => {
    (async () => {
      try {
        // أحياناً يجي النوع type=recovery – هذا المطلوب
        if (!tokens.access_token || !tokens.refresh_token) {
          // قد تكون الجلسة تم ضبطها تلقائياً من قبل؛ نجرب الحصول عليها
          const { data } = await supabase.auth.getSession();
          if (data.session) {
            setReady(true);
            return;
          }
          setError('الرابط غير صالح أو مفقود (access_token). افتحي رابط إعادة التعيين من جديد.');
          return;
        }
